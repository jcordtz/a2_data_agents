...
def generate_agent(
    ...
    try:
        ...
        generate_local_settings(output_path)
        ...
        generate_agent_config(output_path, schema, table_name)
...
def generate_local_settings(output_path: Path):
...
def generate_agent_config(output_path: Path, schema: str, table_name: str):
    """Generate agent configuration file.
    
    Tries to read Azure OpenAI settings from the main project's agent_config.ini,
    otherwise generates a template that needs to be configured.
    """
    import configparser
    
    new_config = configparser.ConfigParser()
    
    # Try to read from main project config
    main_config_path = Path(__file__).parent.parent / "agent_config.ini"
    if main_config_path.exists():
        parser = configparser.ConfigParser()
        parser.read(main_config_path)
        
        # Copy azure_openai section if present
        if "azure_openai" in parser.sections():
            new_config["azure_openai"] = dict(parser["azure_openai"])
    
    # Add default azure_openai section if not present
    if "azure_openai" not in new_config:
